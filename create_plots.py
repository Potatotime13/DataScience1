...
def main():
    ...
    plt.title('Book-Crossing')
    plt.show()
    x = np.arange(len(results_b[0][2].values[0, 1:]))
    plt.bar(x-width, results_b[0][2].values[4, 1:], width, color='cyan')
    plt.bar(x,       results_b[1][2].values[4, 1:], width, color='orange')
    plt.bar(x+width, results_b[2][2].values[4, 1:], width, color='green')
    plt.xticks(x, results_b[0][2].values[0, 1:])
    plt.legend(["user/user pearson", "user/user euclidean", "item/item pearson"])
